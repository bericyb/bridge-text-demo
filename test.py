...
from notification import SnsWrapper
import boto3
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        for result in results:
            for alt in result.alternatives:
                ...
                string = alt.transcript
                if "Annie" in string or "Steve" in string:
                    print("Sending text!")
                    sendNotification(alt.transcript)


def sendNotification(message):
    SnsWrapper.publish_text_message(
        notificationWrapper, "+14804374535", message)
...
notificationWrapper = SnsWrapper(boto3.resource('sns'))
loop = asyncio.get_event_loop()
